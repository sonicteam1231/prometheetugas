...
const PrometheeProses = () => {
  ...
  // --- Membungkus fungsi perhitungan dengan useCallback ---
  const calculatePreferenceIndexes = useCallback(() => {
    const indexes = {};
  ...
  }, [daftarToko, criteriaData]); // Dependensi untuk useCallback

  const calculateMultiCriteriaPreferenceIndexes = useCallback(() => {
    const multiIndexes = {};
    const numberOfCriteria = criteriaData.length;

    if (numberOfCriteria === 0) return;

    for (let i = 0; i < daftarToko.length; i++) {
      for (let j = 0; j < daftarToko.length; j++) {
        const tokoA = daftarToko[i];
        const tokoB = daftarToko[j]; // Perbaikan: dari daftarTaro menjadi daftarToko
        
        if (i === j) {
          multiIndexes[`${tokoA.nama.toLowerCase()},${tokoB.nama.toLowerCase()}`] = '-';
          continue;
        }
        ...
        const phi = valid_criteria_count > 0 ? (sum_H_d / valid_criteria_count) : 0;
        multiIndexes[pairKey] = parseFloat(phi.toFixed(2));
      }
    }
    setMultiCriteriaPreferenceIndexes(multiIndexes);
  }, [daftarToko, criteriaData, preferenceIndexes]); // Dependensi untuk useCallback

  const calculateFlows = useCallback(() => {
    const updatedDaftarToko = daftarToko.map(tokoA => {
      let leavingFlow = 0;
      let enteringFlow = 0;
      const nMinusOne = daftarToko.length - 1;
      ...
      daftarToko.forEach(tokoX => {
        if (tokoA.id !== tokoX.id) {
          const phi_a_x_key = `${tokoA.nama.toLowerCase()},${tokoX.nama.toLowerCase()}`;
          const phi_a_x = multiCriteriaPreferenceIndexes[phi_a_x_key] || 0;
          if (typeof phi_a_x === 'number') {
            leavingFlow += phi_a_x;
          }

          const phi_x_a_key = `${tokoX.nama.toLowerCase()},${tokoA.nama.toLowerCase()}`;
          const phi_x_a = multiCriteriaPreferenceIndexes[phi_x_a_key] || 0;
          if (typeof phi_x_a === 'number') {
            enteringFlow += phi_x_a;
          }
        }
      });

      const finalLeavingFlow = parseFloat((leavingFlow / nMinusOne).toFixed(2));
      const finalEnteringFlow = parseFloat((enteringFlow / nMinusOne).toFixed(2));
      const netFlow = parseFloat((finalLeavingFlow - finalEnteringFlow).toFixed(2));

      return {
        ...tokoA,
        entringFlow: finalEnteringFlow,
        leavingFlow: finalLeavingFlow,
        netFlow: netFlow
      };
    });
    setDaftarToko(updatedDaftarToko);
  }, [daftarToko, multiCriteriaPreferenceIndexes]); // Dependensi untuk useCallback
  // --- Akhir pembungkus useCallback ---


  // --- useEffect yang memicu perhitungan ---
  useEffect(() => {
    ...
    if (daftarToko.length >= 2 && criteriaData.length > 0) {
      ...
      setPreferenceIndexes({});
      setMultiCriteriaPreferenceIndexes({});
      // Reset flows jika tidak cukup data, agar UI konsisten
      setDaftarToko(prev => prev.map(toko => ({ ...toko, entringFlow: 0, leavingFlow: 0, netFlow: 0 })));
    }
  }, [daftarToko.length, criteriaData.length, calculatePreferenceIndexes]); // Tambahkan calculatePreferenceIndexes sebagai dependensi
  ...
  }, [preferenceIndexes, criteriaData.length, calculateMultiCriteriaPreferenceIndexes]); // Tambahkan calculateMultiCriteriaPreferenceIndexes sebagai dependensi

  useEffect(() => {
    if (Object.keys(multiCriteriaPreferenceIndexes).length > 0 && daftarToko.length > 0) {
      calculateFlows();
    }
  }, [multiCriteriaPreferenceIndexes, daftarToko.length, calculateFlows]); // Tambahkan calculateFlows sebagai dependensi
  // --- Akhir useEffect yang memicu perhitungan ---
  ...
  return (
    ...
    {/* Tabel Rangking Promethee Toko - Updated with Flows */}
      <div className="bg-white p-6 rounded-lg shadow-md mb-8">
              ...
              {paginatedToko.length === 0 ? (
                ...
                paginatedToko.map((toko) => (
                  <tr key={toko.id} className="border-b border-gray-200 hover:bg-gray-100">
                    ...
                    {/* Menampilkan nilai flow yang sudah dihitung */}
                    <td className="py-3 px-6 text-left">{toko.entringFlow !== undefined ? toko.entringFlow : 'N/A'}</td>
                    <td className="py-3 px-6 text-left">{toko.leavingFlow !== undefined ? toko.leavingFlow : 'N/A'}</td>
                    <td className="py-3 px-6 text-left">{toko.netFlow !== undefined ? toko.netFlow : 'N/A'}</td>
      ...
      {/* Bagian Tabel Indeks Preferensi (Per Kriteria) */}
      <div className="bg-white p-6 rounded-lg shadow-md mb-8">
      ...
      {/* Bagian Tabel Indeks Preferensi Multikriteria */}
      <div className="bg-white p-6 rounded-lg shadow-md mb-8">
